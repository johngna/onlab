<div>
    <!-- loading -->
    @if($loading)
        <div class="fixed inset-0 z-40 flex items-center justify-center bg-gray-500 bg-opacity-90">
            <div class="flex items-center justify-center w-full h-full">
                <svg class="z-50 w-16 h-16 text-primary-700 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
    ...
    <section class="p-3 antialiased bg-gray-100 dark:bg-gray-900 sm:p-5">
            ...
            <div class="relative overflow-hidden bg-white shadow-md dark:bg-gray-800 sm:rounded-lg">
                <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
                    ...
                    <div class="flex flex-col items-stretch justify-end flex-shrink-0 w-full space-y-2 md:w-auto md:flex-row md:space-y-0 md:items-center md:space-x-3">
                        <button wire:click='syncAuvo' class="flex items-center justify-center px-4 py-2 text-sm font-medium text-white rounded-lg bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800">
                            Sincronizar Clientes AUVO
                        </button>
                    ...
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th scope="col" class="px-4 py-3">Cpf/CNPJ</th>
                        ...
                        <tbody>
                            ...
                            <tr class="border-b dark:border-gray-700">
                                <th scope="row" class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">{{$cliente->description}}</th>
                                <td class="px-4 py-3 max-w-[12rem] truncate">{{$cliente->address}}</td>
                                <td class="px-4 py-3">
                                    @if(is_array(json_decode($cliente->phoneNumber, true)))
                                        @foreach(json_decode($cliente->phoneNumber, true) as $phone)
                                            <div>{{$phone}}</div>
                                        @endforeach
                                    @else
                                        {{$cliente->phoneNumber}}
                                ...
                                <td class="px-4 py-3"> {{$cliente->cpfCnpj}}</td>

                                <td class="px-4 py-3">
                                    @if(is_array(json_decode($cliente->email, true)))
                                        @foreach(json_decode($cliente->email, true) as $email)
                                            <div>{{$email}}</div>
                                        @endforeach
                                    @else
                                        {{$cliente->email}}
                                ...
                                <td class="flex items-center justify-end px-4 py-3">
                                    ...
                                    <div id="apple-imac-27-dropdown" class="z-10 hidden bg-white divide-y divide-gray-100 rounded shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                                        <ul class="py-1 text-sm" aria-labelledby="apple-imac-27-dropdown-button">
                                            <li>
                                                <button type="button" data-modal-target="updateProductModal" data-modal-toggle="updateProductModal" class="flex items-center w-full px-4 py-2 text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white dark:text-gray-200">
                                            ...
                                            <li>
                                                <button type="button" data-modal-target="readProductModal" data-modal-toggle="readProductModal" class="flex items-center w-full px-4 py-2 text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white dark:text-gray-200">
                                                    <svg class="w-4 h-4 mr-2" xmlns="http://www.w3.org/2000/svg" viewbox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                            ...
                                            <li>
                                                <button type="button" data-modal-target="deleteModal" data-modal-toggle="deleteModal" class="flex items-center w-full px-4 py-2 text-red-500 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-red-400">
    ...
    <!-- End block -->
    <!-- Create modal -->
    <div id="createProductModal" tabindex="-1" aria-hidden="true" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full" wire:ignore>
        <div class="relative w-full max-w-2xl max-h-full p-4">
            <!-- Modal content -->
            <div class="relative p-4 bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                <!-- Modal header -->
                <div class="flex items-center justify-between pb-4 mb-4 border-b rounded-t sm:mb-5 dark:border-gray-600">
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Adicionar Equipamento</h3>
                ...
                <form action="#">
                    <div class="grid gap-4 mb-4 sm:grid-cols-2">
                        <div>
                            <x-input label="Número de Série" placeholder="Número de Série" wire:model='equipamento.numero_serie' />
                        </div>
                        <div class="flex items-end">
                            <x-button label="Buscar no AUVO" wire:click='getEquipAuvo' class="justify-center" />
                        </div>
                        <div class="sm:col-span-2">
                            <x-input label="Nome" placeholder="Nome do Equipamento" wire:model='equipamento.nome' />
                        </div>

                        <div>
                            <x-input label="Cliente" placeholder="Cliente" wire:model='equipamento.cliente' />
                        </div>
                        <div>
                            <x-input label="Modelo" placeholder="Modelo" wire:model='equipamento.modelo' />
                        </div>
                        <div class="sm:col-span-2">
                            <x-textarea label="Descrição" placeholder="" wire:model='equipamento.descricao' />
                        </div>
                    </div>
        ...
        <div class="relative w-full max-w-md max-h-full p-4">
            ...
            <div class="relative p-4 text-center bg-white rounded-lg shadow dark:bg-gray-800 sm:p-5">
                ...
                <div class="flex items-center justify-center space-x-4">
                    ...
                    <button type="submit" class="px-3 py-2 text-sm font-medium text-center text-white bg-red-600 rounded-lg hover:bg-red-700 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-900">Yes, I'm sure</button>
                </div>
            </div>
        </div>
    </div>
